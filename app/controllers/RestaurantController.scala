...
class RestaurantController @Inject extends Controller{
  ...
  implicit val restaurantReads:Reads[Restaurant]=(
    ...
    RestaurantsDAO.list map {
      restaurant => val json=Json.toJson(restaurant)
      Ok(json)
    }
  }
  
  def getById(id: Long) = Action.async {
    RestaurantsDAO.getById(id) map { restaurant =>
    ...
    RestaurantsDAO.save(cRestaurant) map { res =>
      Ok(res)
    }
  }
    ...
    RestaurantsDAO.update(cRestaurant) map { res =>
      Ok("Updated "+res)  
    }
  }
    
    def delete(id:Long) =Action.async{
    RestaurantsDAO.delete(id) map { deleted =>
      Ok("Deleted "+deleted)      
    }
  }
